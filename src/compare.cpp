...
float Compare::non_holonomic_without_obs(State src) {
  ...
  CE.x = Compare::target.dx +
         (rmin)*sin((Compare::target.theta) * PI / 180);  // Clockwise-End
  CE.y = Compare::target.dy - (rmin)*cos((Compare::target.theta) * PI / 180);

  ACE.x = Compare::target.dx -
          (rmin)*sin((Compare::target.theta) * PI / 180);  // Anti-Clockwise-End
